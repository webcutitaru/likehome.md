...
/**
 * Sitemap XML pentru Google Search Console (pagini publice + proprietăți active).
 */
...
foreach ($static as $row) {
    $loc = $row['path'] === '' ? lh_absolute_url('') : lh_absolute_url($row['path']);
    echo '  <url>';
...
try {
    $pdo = getPDO();
    $stmt = $pdo->query(
        'SELECT id, slug, created_at FROM properties WHERE is_active = 1 ORDER BY id ASC'
    );
    if ($stmt) {
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $prop) {
            $slug = trim((string) ($prop['slug'] ?? ''));
            $q = $slug !== ''
                ? http_build_query(['slug' => $slug])
                : http_build_query(['id' => (int) ($prop['id'] ?? 0)]);
            $loc = lh_absolute_url('property-details.php?' . $q);
