<?php

declare(strict_types=1);

require_once __DIR__ . '/config.php';
require_once __DIR__ . '/includes/page_i18n.php';
require_once __DIR__ . '/includes/site_nav.php';

$privacy = lh_page_sections('privacy');
$sections = is_array($privacy['sections'] ?? null) ? $privacy['sections'] : [];
$pageTitle = __('page.privacy.title');
$pageDescription = __('page.privacy.description');
$canonicalUrl = lh_absolute_locale_url('privacy.php');
$robotsMeta = 'index, follow, max-snippet:-1';
$contactEmail = lh_site_contact_email();
$contactCity = lh_site_contact_city();
?>
<?php include __DIR__ . '/includes/header.php'; ?>

<div class="border-b border-black/[0.06] bg-white/60">
  <div class="max-w-4xl mx-auto px-4 sm:px-6 pt-10 md:pt-14 pb-6 md:pb-8">
    <p class="text-xs font-semibold uppercase tracking-[0.2em] text-blue-grey mb-3"><?= htmlspecialchars((string) ($privacy['label'] ?? ''), ENT_QUOTES, 'UTF-8') ?></p>
    <h1 class="text-3xl md:text-4xl font-bold text-ink tracking-tight"><?= htmlspecialchars(__('page.privacy.heading'), ENT_QUOTES, 'UTF-8') ?></h1>
    <p class="mt-4 text-sm text-blue-grey">
      <?= htmlspecialchars(__('page.privacy.updated'), ENT_QUOTES, 'UTF-8') ?> <?= htmlspecialchars(date('d.m.Y'), ENT_QUOTES, 'UTF-8') ?>.
      <?= htmlspecialchars((string) ($privacy['updated_note'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
    </p>
  </div>
</div>

<div class="max-w-4xl mx-auto px-4 sm:px-6 mt-6 md:mt-10 pt-0 pb-0">
  <div class="space-y-10 text-blue-grey leading-relaxed [&_h2]:text-ink [&_h2]:text-xl [&_h2]:font-bold [&_h2]:mt-0 [&_h2]:mb-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:space-y-2">
<?php foreach ($sections as $section):
    if (!is_array($section)) {
        continue;
    }
    $body = str_replace(
        ['{email}', '{city}'],
        [$contactEmail, $contactCity],
        (string) ($section['body'] ?? '')
    );
?>
    <section>
      <h2><?= htmlspecialchars((string) ($section['title'] ?? ''), ENT_QUOTES, 'UTF-8') ?></h2>
      <?= $body ?>
    </section>
<?php endforeach; ?>
  </div>
</div>

<?php include __DIR__ . '/includes/footer.php'; ?>
